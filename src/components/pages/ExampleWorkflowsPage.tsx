...
export default function ExampleWorkflowsPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-paragraph">
      ...
      <section className="w-full pt-32 pb-16 border-b border-accent-grey">
      ...
      {/* WORKFLOW EXAMPLES - COMPACT GRID */}
          ...
          <div className="min-h-[400px]">
            {isLoadingData ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="bg-accent-grey/30 h-48 animate-pulse rounded-lg"></div>
                ))}
              </div>
              ...
              <div className="text-center py-20">
                <p className="text-secondary">No workflow examples available.</p>
              </div>
            )}
          </div>
        </div>
...
// Compact card component - minimal design
function CompactProcessCard({ process, index }: { process: ProcessExamples, index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-accent-grey p-6 group hover:border-primary hover:shadow-md transition-all duration-300 flex flex-col justify-between min-h-[200px]"
    >
      <div>
        <div className="flex justify-between items-start gap-3 mb-4">
          <h3 className="font-heading text-lg text-foreground group-hover:text-primary transition-colors leading-tight">
            {process.processName}
          </h3>
          <Plus className="w-5 h-5 text-accent-grey group-hover:text-primary group-hover:rotate-90 transition-all duration-300 flex-shrink-0 mt-0.5" />
        </div>
      ...
      {process.commonPainPoint && (
        <div className="pt-4 mt-4 border-t border-accent-grey/50">
          <p className="font-paragraph text-xs text-secondary/70">
            <span className="font-bold text-foreground/70">Pain Point:</span> {process.commonPainPoint}
          </p>
        </div>
      )}
    </motion.div>
